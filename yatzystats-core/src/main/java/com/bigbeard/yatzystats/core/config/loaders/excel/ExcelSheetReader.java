...
public class ExcelSheetReader implements SheetReader {
    ...
    @Override
    public String getSheetYear() throws CellNotFoundException {
        Cell cell = facade.readCell(this.sheet, 1, 0);
        String sheetYear = cell.getStringCellValue();
        if (sheetYear.length() >= 4) {
            return sheetYear.substring(sheetYear.length() - 4);
        } else {
            return sheetYear;
        }
    }
